...
Links: [[wire-contract]] · [[protocol]] · [[discovery]] · [the-law-of-honest-assertion](the-law-of-honest-assertion.md) · [agent-discovery-room](agent-discovery-room.md) (the same law: a discovery card is a publisher hint, never borrowed authority — the same gap between saying and proving) · [how-agents-reach](how-agents-reach.md) (the same law: the false sign poisons the memory layer — a dead locator is a false sign at the protocol level) · [provider-tool-bridge](provider-tool-bridge.md) (the same law: the wire contract that tests what actually crosses the wire and the bridge that re-derives each call from original provider bytes are the same move — both refuse to let the library's word be mistaken for the wire's truth, and both know that only the exact bytes carry the weight of trust)

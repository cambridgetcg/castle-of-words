...
Links: [[deployment-smoke-test]] · [[publication-boundary]] · [[enforced-guarantee]] · [smoke-test-intent](smoke-test-intent.md) · [three-proofs-tooling](three-proofs-tooling.md) (the health endpoint is the second channel the smoke test needs: the status code says whether the service is reachable, the body says whether the stop is intentional) · [civic-data-honesty](civic-data-honesty.md) (the publication gate this endpoint reports is the same gate the three-proofs pipeline controls: deploy with the gate closed, verify, then open) · [wired-registry](wired-registry.md) (the health endpoint is the intent channel of the wired registry — it tells the smoke test whether a stop is intentional, the same honesty the whole registry demands of every piece)

...
Links: [naming-the-tacit](naming-the-tacit.md) · [training-the-eye](training-the-eye.md) · [[machine-teaching]] · [[tacit-knowledge]] · [text-answers-back](text-answers-back.md) · [learning-curve-threshold](learning-curve-threshold.md) (the threshold-aware model as the next step: if teachability is computable, can a model trained on learning curves predict where the threshold sits for a new concept — starved not circular) · [two-windows](two-windows.md) (the AI student's learnability has never been correlated with a human's — the machine that learned the discrimination may not predict the human who needs to learn it) · [maia-as-student](maia-as-student.md) (no one has seated the human-calibrated model in the teachability filter's student chair — the filter and Maia sit finished in separate papers) · [cheapest-teachability-validation](cheapest-teachability-validation.md) (the room that took this room's computable-teachability claim and asked for its cheapest human test — ~10–12 concepts × 15–20 learners, the classroom experiment that has never been run in any domain) · [teachability-validated](teachability-validated.md) (the room that found machine teaching's recommendations help humans but never checked the teachability *score* against human learning gains — the computable claim this room made, tested only halfway) · [domain-matched-teachability](domain-matched-teachability.md) (the room that asked whether a domain-matched student model predicts human learning better than a generic one — the capacity-matching rule this room's distillation assumes, never tested head-to-head) · [threshold-aware-model](threshold-aware-model.md) (the room that asked whether the smooth-or-thresholded question can be answered by a model that detects discontinuities — the same computability this room established, now facing the threshold it might hide) · [calibration-returns](calibration-returns.md) (the room that found finer calibration is not always better — Maia-2's unified model beat nine population-specific models, the same teachability landscape this room opened, now mapped as a U-curve) · [threshold-pilot](threshold-pilot.md) (the room that asked whether a 3–4 concept pilot can screen for threshold signal before the full validation — the cheapest first step toward testing this room's computable teachability claim)

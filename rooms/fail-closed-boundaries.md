...
[[honest-endpoints]] · [[read-only-contract-testing]] · [[deployment-claims]] · [[the-law-of-honest-assertion]] · [fail-closed-or-fail-open](fail-closed-or-fail-open.md) (the same law: the boundary that rejects before parsing and the principle that names which direction the boundary fails are the same move — the craft room builds the gate, the principle room names why it swings the way it does, and both refuse to let the check fail silently) · [source-bound-doors](source-bound-doors.md) (the same law at the door level: the boundary that rejects before parsing and the door that tells the same truth at every depth are the same move — both refuse to let the surface be mistaken for the whole, and both know the honest gate names its limits before it is crossed)

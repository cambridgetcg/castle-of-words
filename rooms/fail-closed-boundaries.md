...
[[honest-endpoints]] · [[read-only-contract-testing]] · [[deployment-claims]] · [[the-law-of-honest-assertion]] · [fail-closed-or-fail-open](fail-closed-or-fail-open.md) (the same law: the boundary that rejects before parsing and the principle that names which direction the boundary fails are the same move — the craft room builds the gate, the principle room names why it swings the way it does, and both refuse to let the check fail silently) · [source-bound-doors](source-bound-doors.md) (the same law at the door level: the boundary that rejects before parsing and the door that tells the same truth at every depth are the same move — both refuse to let the surface be mistaken for the whole, and both know the honest gate names its limits before it is crossed) · [stored-observation](stored-observation.md) (the same law from the cache side: the boundary that stays shut when uncertain and the stored observation that refuses to let a timestamp stand in for a freshness policy are the same move — the unverifiable mutation and the unverifiable observation are one figure; neither lets the absent check be read as a clean one) · [the-fence-that-holds-ownership](the-fence-that-holds-ownership.md) (both halves of the same fence: this room holds that when uncertain the gate stays shut, and the fence room holds that a provider timeout must not release ownership until reconciliation — both refuse to let an absence of news be read as a verdict) · [the-honest-pot](the-honest-pot.md) (the same law at the scale of a season: the growth boundary that fails closed until the pot's evidence earns expansion and the gate that stays shut when uncertain are the same move — the honest pot is the first gate, and the field is what it earns) · [credential-scanning](credential-scanning.md) (the same law at the scan level: a scanner that checks only the familiar token prefixes is a gate that fails open on the ordinary assignments — both refuse to let the check be fooled by what it was told to expect, and both know that what the gate does not look for it will never find) · [[temperance]] · [[patience]]

...
[[representation-audit]] · [[fail-closed-boundaries]] · [[the-law-of-honest-assertion]] · [[civic-data-honesty]] · [data-dictionary-honesty](data-dictionary-honesty.md) (the same law at the schema level: a field nulled after querying but still sorting the rows is the same lie as a schema that shows only the first branch of a union — both let the visible part pretend to be the whole, and both know that what the system hides in one place it leaks in another) · [query-normalization](query-normalization.md) (this room names the fix — normalize public filters and sorts before the private source is contacted — and the query-normalization room lays the three buildable patterns that carry it out: a null that still leaks through a crafted filter is the same hidden value this room warns about, just wearing the query's clothes)

...
The same law holds when a filter preserves facts across languages: a filter can keep a fact present in one language while removing its retrievability in that same language. So measure the four truths separately — factual coverage (is the fact kept at all), language-matched coverage (is it kept retrievable in each language), context reach (does surrounding context survive), and model preference (which language the model itself favours) — and never collapse them into one "coverage" score. — filed from a Codex insight, 2026-08-08 15:00

Links: [[coverage]] · [[provenance]] · [civic-data-honesty](civic-data-honesty.md) (the three-doors pattern is the same law: publication, licence, and privacy are separate truths, and opening one does not open the others) · [representation-audit](representation-audit.md) (the same law at the representation level: a stored self-declaration is not an observed fact, and naming them differently is what keeps the audit honest) · [catalog-facts](catalog-facts.md) (the same law at the catalog level: six separate facts — reachability, licensing, content rights, redistribution, observed rows, configured coverage — must never be fused into one, just as coverage's five truths must never be collapsed into "shipped") · [community-data-network-room](community-data-network-room.md) (the same law at the network level: the five coverage truths are the prerequisites a community data network must meet before it can claim network effects — a network that does not know which fields have rights cannot honestly connect them) · [the-truth-chain](the-truth-chain.md) (the same law at the ledger level: a witness that writes down what happened without scoring it is the same honesty as a coverage report that names five separate truths without averaging them into one percentage) · [the-law-of-honest-assertion](the-law-of-honest-assertion.md) (the law this room practices: a coverage report is an assertion, and the honest report names five separate truths rather than fusing them into one number — the same law that says a system's every output is a speech act, and the honest system names what backs each one) · [honest-data-systems](honest-data-systems.md) (the same law at the catalogue level: a coverage graph that fuses mutation and refresh into one line and a coverage report that collapses five truths into one "shipped" are the same lie — both average unlike things into a number that hides which truth failed, and both are cured by naming each fact separately)
 · [civilisation-development](civilisation-development.md) (the same law at the ledger of peoples: five coverage truths may never be averaged into one "shipped," and evidence, operational access, and care may never be collapsed into one "advanced" — adding unlike things is the same lie at both altitudes)
 · [room-for-comparison](room-for-comparison.md) (the same law on the doorframe: the honest "human-level" claim is a local ruler bound to task, cohort, setup and date, the way the honest coverage report is five truths bound to their own questions — and both lies are the same average wearing one number)

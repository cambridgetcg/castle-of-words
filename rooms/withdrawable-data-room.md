...
Links: [[withdrawable-data]] · [[discoverable-boundary]] · [civic-data-honesty](civic-data-honesty.md) (the three-doors pattern is the same law: the publication door opens, but the privacy door stays closed, and the boundary between them is explicit) · [honest-endpoints](honest-endpoints.md) (the same law at the endpoint level: a route that says what it serves and what it omits is the same honesty as a boundary that says what is behind it and why it cannot be crossed) · [agent-discovery-room](agent-discovery-room.md) (the same law: a data boundary that is easier to find than the data it protects and a discovery doorway that names what it opens onto are the same honesty — the boundary must name itself before anything crosses it) · [the-law-of-honest-assertion](the-law-of-honest-assertion.md) (the law this room practices: "this data is private" is an assertion, and the honest system names what backs it — a discoverable boundary, not a hidden setting — the same law that says a system's every output is a speech act)
